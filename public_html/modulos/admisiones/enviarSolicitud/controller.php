...
require_once('../../admisiones/verSolicitud/model.php');
require_once('../../admisiones/documentos_admision/model.php');
...
{   $solicitud 		= get_mainObject('Solicitud');
	...
	{   case VIEW_GET_ALL:
			...
			$periodo->periodo_activo_admisiones( );
		...
		case FORMULARIO:
			...
			$data['fotoUsuario'] = '../../'.$_SESSION['ruta_foto_usuario'].'admin.jpg';
		...
		case FORMULARIO_CONTINUAR:
			...
			{   $solicitud->get_solicitud_persona_consulta( $user_data['id_solicitud'], $user_data['estudiante_per_codi'] );
				...
				{	/*-------------------------------------------------------------------------------------
					...
					if ( ( $solicitud->rows[0]['per_sector_residencia'] == '' ) )
						$data['per_sector_residencia_fuera_disabled'] = " disabled='disabled' ";
		...
		case GET_SOLICITUD_ESPECIFICA:
			...
			{   $data=' <div class="callout callout-info">
							...
							ID DE LA SOLICITUD: "'.$solicitud->rows[0]['soli_codi'].'". ESTADO: "'.$solicitud->rows[0]['soli_estado'].'" 
							<br>
						</div>';
				...
				{   $solicitud_fecha->solicitud_fecha_examen_ver( $user_data['id_solicitud'] );
					...
					{	$data.= "<div style='background-color:#e5e5e5;height:300px;overflow-y:scroll;'>";
						...
						$data.= "	<div class='callout callout-success'>
										<b>Fechas de exámenes/actividades asignadas</b><br><br>".
				...
				{   $data.= '<div>
								<a id="btn_continuar_solicitud" name="btn_continuar_solicitud" class="btn btn-app"
									onclick="js_enviarSolicitud_carga_formulario(\''.$solicitud->rows[0]['soli_codi'].'\',\''.$solicitud->rows[0]['per_codi'].'\',\'div_formulario_solicitud\',\''.$diccionario['rutas_head']['ruta_html_admisiones'].'/enviarSolicitud/controller.php\', 0 )">
		...
		case ENVIAR_SOLICITUD:
			...
			break;
		case VALIDAR_CORREO:
			if (!filter_var($user_data['value'], FILTER_VALIDATE_EMAIL)) {
        ...
        break;
		case VALIDAR_ESNUMERICO:
			$value = $user_data['value'];
			if (!is_numeric($value)) {
				$message= "Ingrese solo números por favor";
				$result = "error";
			}
			else{
				if (strlen($value)<5){
		...
		case VALIDAR_IDENTIFICACION:
			$strCedula = $user_data['value'];
			$tipo_iden = $user_data['type'];
			$result		= "";
			$message	= "";
...
{   if ( $curso == '' )
	...
	$data['{cmb_pais_per_lugar_nac}'] = array("elemento"  => 	"combo", 
									...
									"options"   => 	array(	"name"		=>"cmb_pais_per_lugar_nac",
															...
															"required"	=>"required",
															"class"		=>"form-control",
															"onChange"	=> "js_region_cargaCiudadDistrito('div_ciudad_per_lugar_nac','div_provincia_per_lugar_nac','cmb_ciudad_per_lugar_nac','cmb_provincia_per_lugar_nac','cmb_pais_per_lugar_nac','".$diccionario['rutas_head']['ruta_html_common']."/region/controller.php')"),
									"selected"  => 	$pais_nac);
	$data['{cmb_provincia_per_lugar_nac}'] = array("elemento"  => 	"combo", 
									...
									"options"   => 	array(	"name"		=>"cmb_provincia_per_lugar_nac",
															...
															"required"	=>"required",
															"class"		=>"form-control",
															"onChange"	=>"js_region_cargaCiudad('div_ciudad_per_lugar_nac','cmb_ciudad_per_lugar_nac','cmb_provincia_per_lugar_nac','cmb_pais_per_lugar_nac','".$diccionario['rutas_head']['ruta_html_common']."/region/controller.php')"),
									"selected"  => 	$prov_nac);
	...
	$data['{cmb_pais_colegio_anterior}'] = array(	"elemento"  => 	"combo", 
													...
													"options"   => 	array(	"name"		=>"cmb_pais_colegio_anterior",
																			...
																			"required"	=>"required",
																			"class"		=>"form-control",
																			"onChange"	=> "js_region_cargaCiudadDistrito('div_ciudad_colegio_anterior','div_provincia_colegio_anterior','cmb_ciudad_colegio_anterior','cmb_provincia_colegio_anterior','cmb_pais_colegio_anterior','".$diccionario['rutas_head']['ruta_html_common']."/region/controller.php')"),
													"selected"  => 	$pais_cole);
	$data['{cmb_provincia_colegio_anterior}'] = array(  "elemento"  => 	"combo", 
														...
														"options"   => 	array(	"name"		=>"cmb_provincia_colegio_anterior",
																				...
																				"required"	=>"required",
																				"class"		=>"form-control",
																				"onChange"	=>"js_region_cargaCiudad('div_ciudad_colegio_anterior','cmb_ciudad_colegio_anterior','cmb_provincia_colegio_anterior','cmb_pais_colegio_anterior','".$diccionario['rutas_head']['ruta_html_common']."/region/controller.php')"),
														"selected"  => 	$prov_cole);
...
{	
	...
	$data['{cmb_pais_'.$repr_ctrl.'_lugar_nac}'] = array(	"elemento"  => 	"combo", 
															...
															"options"   => 	array(	"name"		=>"cmb_pais_".$repr."_lugar_nac",
																					...
																					"required"	=>"required",
																					"class"		=>"form-control",
																					"onChange"	=> "js_region_cargaCiudadDistrito('div_ciudad_".$repr."_lugar_nac','div_provincia_".$repr."_lugar_nac','cmb_ciudad_".$repr."_lugar_nac','cmb_provincia_".$repr."_lugar_nac','cmb_pais_".$repr."_lugar_nac','".$diccionario['rutas_head']['ruta_html_common']."/region/controller.php')"),
															"selected"  => 	$pais_nac);
	$data['{cmb_provincia_'.$repr_ctrl.'_lugar_nac}'] = array(  "elemento"  => 	"combo", 
																...
																"options"   => 	array(	"name"		=>"cmb_provincia_".$repr."_lugar_nac",
																						...
																						"required"	=>"required",
																						"class"		=>"form-control",
																						"onChange"	=>	"js_region_cargaCiudad('div_ciudad_".$repr."_lugar_nac','cmb_ciudad_".$repr."_lugar_nac','cmb_provincia_".$repr."_lugar_nac','cmb_pais_".$repr."_lugar_nac','".$diccionario['rutas_head']['ruta_html_common']."/region/controller.php')"),
														"selected"  => 	$prov_nac);
	...
	{   $array1[]= '
				<div class="{columna}">
					<div class="small-box bg-'.$colors[$i].'" id="div_peri'.$i.'" name="div_peri'.$i.'"">
						<div class="inner">
							<h2>'.$row['peri_deta'].'<sup style="font-size: 20px"></sup></h2>
							<p>Del: '.date_format($row['peri_fech_ini'],"d-m-y").' - Hasta: '.date_format($row['peri_fech_fin'],"d-m-y").'</p>
						</div>
						<div class="icon">
							<i class="ion ion-calendar"></i>
						</div>
						<a href="#" class="small-box-footer" onclick="js_general_change_periodo_2('.$row['peri_codi'].',\''.$row['peri_deta'].'\',\''.$diccionario['rutas_head']['ruta_html_admisiones'].'/general/controller.php\')">Haz clic aqu&iacute; <i class="fa fa-arrow-circle-right"></i></a>
					</div>
				</div>';
			$i++;
	}
