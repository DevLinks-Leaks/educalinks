...
class General extends DBAbstractModel{
	...
	{	$this->parametros = array("Y");
		$this->sp = "peri_acti_etap";
		$this->executeSPConsulta();
        if (count($this->rows)>1)
		{	array_pop($this->rows);
			foreach($this->rows[0] as $propiedad=>$valor)
	...
	public function get_bandejas_act( )
	{	$this->parametros = array( );
		$this->sp = "str_admisiones_bandejas_act";
		$this->executeSPConsulta();
		if (count($this->rows)>1)
		{	$bandejas_act = array();
            foreach($this->rows as $bandejas_acts)
			{	array_push($bandejas_act, array_values($bandejas_acts));
            }
            // Agregar la opcion de ninguna categoria padre
            array_pop($permiso);
            array_push($permiso, array());

            $this->rows = $bandejas_act;
            unset($bandejas_act);
        }
		else
		{	//$this->mensaje=$this->login_mensaje;
        }
			$this->bandejas_act=$this->rows;
	}
