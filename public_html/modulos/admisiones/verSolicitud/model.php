...
class Solicitud extends DBAbstractModel{
	...
	{   $this->parametros = array( $soli_codi );
		$this->sp = "str_admisiones_solicitudes_estados_historial";
        $this->executeSPConsulta();
	...
	{	$this->parametros = array( );
        ...
        $this->query = "delete from tbl_solicitud where soli_codi = '".$id_solicitud."';
						delete from tbl_solicitud_fecha_examen where soli_codi = '".$id_solicitud."';
						delete from tbl_historial_estado where soli_codi = '".$id_solicitud."';
						delete from tbl_solicitud_sintesis where soli_codi = '".$id_solicitud."';
						delete from tbl_solicitud_repr_inf_adicional where soli_codi = '".$id_solicitud."';
						delete from tbl_solicitud  where soli_codi = '".$id_solicitud."';";
        $this->executeSentenciaConsulta();
        if (count($this->rows)>0)
	...
	public function set_solicitud_estado( $id_solicitud, $peri_codi, $estado, $soli_observacion = "", $usua_ingr, $ip )
	{	$this->id_solicitud_out = 0;
		$this->parametros =	array( $id_solicitud, $peri_codi, $estado, $soli_observacion, $usua_ingr, $ip );
        $this->sp = "str_admisiones_solicitud_estado";
        $this->executeSPConsulta();
        if (count($this->rows)>0)
        {   if ($this->rows[0]['PAGO']=="no_error")
				$this->mensaje="¡Exito! Solicitud guardada. Su código es: '".$this->rows[0]['ID_SOLICITUD']."'.";
			else
				$this->mensaje= "No ha realizado el abono correspondiente. Por favor acérquese a tesorería.";
