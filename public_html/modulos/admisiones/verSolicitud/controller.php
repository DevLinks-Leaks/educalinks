...
function handler() {
	...
	$event 			= get_actualEvents(array(VIEW_GET_ALL, GET_ALL, VIEW_SET, VIEW_SET_HOME), VIEW_GET_ALL);
  ...
  //$permiso = get_mainObject('General');
	...
	{   case VIEW_GET_ALL:
		...
		case PRINTREP_OBS_PDF:
			...
			//$pdf->Image($diccionario['rutas_head']['ruta_imagenes'].'includes/common/logos/LOGO_EDUCALINKS_white.png', 'C', 15, '', '', 'JPG', '', 'C', false, 300, 'C', false, false, 0, false, false, false);
...
{   $html = $rango_trabajo = $v_estado_civil = $profesion_detalle = $pais_nacionalidad = '';
	...
	$html.='<tr><td><b>¿HA TRABAJADO ANTES EN ESTA INSTITUCIÓN?:</b> '. $soli_reprInfo_es_extrabajador . '</td>
...
{	global $diccionario; $mant = 0;
	...
	*/
	...
	if ($soli_estado == 'ADMITIDO')
		$body.= "<th style='text-align:center;font-size:small;'>Migrado</th>";
	$body.="</thead>";
	$body.="<tbody>";
	...
	{	if($c<($aux-1))
		{	$body.= "<tr>";
			...
			{	if( $x == 0 ) $soli_codi = $column;
				...
				if( ( $x != 0 ) && ( $x != 1 ) && ( $x != 2 ) && ( $x != 3 ) && ( $x != 7 ) && ( $x != 10 ) && ( $x != 11 ) )
				{   $body.="<td style='text-align:center;font-size:small;'>".$column."</td>";
				}
				$x++;
			}
			...
			if ($soli_estado == 'ADMITIDO')
			{   if ($migrado == 1)
					$body.= "<td style='text-align:center;font-size:small;color:green;'>SÍ</td>";
				else
					$body.= "<td style='text-align:center;font-size:small;color:red;'>NO</td>";
			}
		}
		$body.="</tr>";
		$c++;
...
{	if($type=='span')
	...
	$opciones.= "&nbsp;&nbsp;<".$type." ".$tag." onclick='js_verSolicitud_observacion_set(\"".$soli_codi."\",".'"'.$ruta.'/verSolicitud/controller.php"'.")' class='fa fa-eye cursorlink' aria-hidden='true' data-toggle='modal' data-target='#modal_obs'  id='".$soli_codi."_asignar_repr' onmouseover='$(this).tooltip(".'"show"'.");'  title='Agregar observación' data-placement='left'>".$space."</".$type.">";
	return $opciones;
}
