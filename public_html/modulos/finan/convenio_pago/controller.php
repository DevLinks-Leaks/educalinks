...
function handler() {
    ...
    switch ($event) {
        ...
        case VIEW_GET_ALL:
            ...
            $factura->get_facturas('','',$tomorrow->format('d/m/Y'),'','','','','','','','','','P',0,0,'-1','0','-1','-1',NULL,NULL,'','',1);
            $data['mensaje'] = "Facturas de convenio de pago por autorizar mensualmente.";
	...
	{	if($c<($aux-1))
		{	$body.="<tr id='tr_row_{codigoDocumento}' name='tr_row_{codigoDocumento}'><td id='td_select_".$c."' name='td_select_".$c."' align='center'><div style='font-size:x-small;'>".
			...
			{	if($x==1)
				...
				elseif($x==6)
				{	$onclick=" onclick='js_convenio_pago_editar(".'"'.$codigo.'"'.",".'"'.$row['codigoAlumno'].'"'.",".'"'.$row['es_alumno'].'"'.",".'"modal_edit_body"'.",".'"'.$diccionario['rutas_head']['ruta_html_finan'].'/gestionFacturas/controller.php"'.",".'"'.$diccionario['rutas_head']['ruta_html_common'].'/representantes/controller.php"'.",true)' ";
			...
			{ 	$spanAuto="<div align='center' style='display:inline-block;'><span   onclick='js_convenio_pago_envio_factura(".'"'.$codigo.'"'.",".'"modal_send_body"'.",".'"'.$diccionario['rutas_head']['ruta_html_finan'].'/gestionFacturas/controller.php"'.", 1, \"".$estadoFac."\")' style='color:red' class='btn_opc_lista_subir_SRI icon icon-sri cursorlink'  aria-hidden='true' data-toggle='modal' data-target='#modal_send'   id='".$codigo."_enviar'   onmouseover='$(this).tooltip(".'"show"'.")' data-placement='left' title='Enviar factura a autorizar al SRI.'></span></div>";
				$spanReen="<div align='center' style='display:inline-block;'><span onclick='js_convenio_pago_reenvio_factura(".'"'.$codigo.'"'.",".'"modal_send_body"'.",".'"'.$diccionario['rutas_head']['ruta_html_finan'].'/gestionFacturas/controller.php"'.", \"".$estadoFac."\")'    style='color:red' class='btn_opc_lista_reenviar glyphicon glyphicon-repeat cursorlink' aria-hidden='true' data-toggle='modal' data-target='#modal_send' id='".$codigo."_reenviar' onmouseover='$(this).tooltip(".'"show"'.")' data-placement='left' title='Reintentar autorizaci&oacute;n de factura.'></span></div>";
			}
			if ($permiso1==true)
