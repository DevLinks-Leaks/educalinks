...
{   require('../../../core/rutas.php');
	...
	{   case VIEW_GET_ALL:
        ...
        case GET_DEUDAS:
            ...
            $opciones = array("Seleccionar" => "<span onclick='js_descuentofactura_carga_asignacion(".'"{codigo}"'.",".'"resultado"'.",".'"'.$diccionario['rutas_head']['ruta_html_finan'].'/descuentofacturas/controller.php"'.")' class='btn_opc_lista_credit_card fa fa-edit cursorlink' aria-hidden='true' data-toggle='modal' id='{codigo}_asignar'onmouseover='$(".'"#{codigo}_asignar"'.").tooltip(".'"show"'.")' title='Editar configuración' data-placement='left'></span>");
		...
		case VIEW_SET_DISCOUNT:
			...
			{   if ( !empty ($rows) )
				{	$tabla_detalle.="
						<tr>
							...
							<td style='text-align:center;'><input type='checkbox' id='ckb_pp_".$rows['detafact_secuencia']."' name='ckb_pp_".$rows['detafact_secuencia']."' ".
							( $rows['prod_prontopago'] == 0 ? '' : 'checked' )."></td>
							<td style='text-align:center;'><input type='checkbox' id='ckb_des_".$rows['detafact_secuencia']."' name='ckb_des_".$rows['detafact_secuencia']."' ".
							( $rows['prod_descuento'] == 0 ? '' : 'checked' )."></td>
							<td style='text-align:center;'><input type='checkbox' id='ckb_liq_".$rows['detafact_secuencia']."' name='ckb_liq_".$rows['detafact_secuencia']."' ".
							( $rows['prod_liquidez'] == 0 ? '' : 'checked' )."></td>
							<td style='text-align:center;'><input type='checkbox' id='ckb_aIVA_".$rows['detafact_secuencia']."' name='ckb_aIVA_".$rows['detafact_secuencia']."' ".
							( $rows['prod_aplicaIVA'] == 0 ? '' : 'checked' ).">
							<span style='display:none;'><input type='checkbox' id='ckb_aICE_".$rows['detafact_secuencia']."' name='ckb_aICE_".$rows['detafact_secuencia']."' ".
			...
			{   if ( !empty ($rows) )
				{	if ( $rows['desc_aplicaprontopago'] == 0)
						$general_check = 0;
					/* Check si se quiere manejar los checks de aplicaprontopago individual.
					<td style='text-align:center;'><input type='checkbox' id='ckb_dapp_".$rows['detafact_desc_codigo']."' name='ckb_dapp_".$rows['detafact_desc_codigo']."' ".
							( $rows['desc_aplicaprontopago'] == 0 ? '' : 'checked' )."></td>*/
					$tabla.="
						<tr>
							<td style='text-align:center;' data-codigo='".$rows['detafact_desc_codigo']."' data-estado='active'>".$rows['desc_descripcion']."</td>
							...
							<td style='text-align:center;'>
								<span onclick='js_descuentofactura_del_descuento_asignado(\"".$rows['desc_codigo']."\")' 
									...
									aria-hidden='true' onmouseover='$(this).tooltip(".'"show"'.")' title='Quitar descuento' data-placement='left'></span></td>";
					$tabla.="</tr>";
					$total_descuentos++;
				}
			}
