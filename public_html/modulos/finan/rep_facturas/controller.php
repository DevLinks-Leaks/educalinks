...
function handler() {
    ...
    switch ($event) {
		...
		case PRINTREP_CIERRES:
			...
			$html .='<table width="100%" cellspacing="0" cellpadding="2" border="0"><tr><td><h2>Reporte de Facturas emitidas - Por Nivel Económico</h2></td>';
			...
			for($i=0;$i<count($reporte->rows)-1;$i++){
				if($item_actual!=$tranx[$i]['niveEcon']){
					if($i!=0){
					...
					$html.='<tr>
						<th align="left"   	style="font-size:8;width:9%">'.$tranx[$i]['niveEcon'].'</th>
		...
		case PRINTREP_CIERRES_XLS:
			...
			{   if($item_actual!=$tranx[$i]['niveEcon'])
				{   if($i!=0)
					...
					{   $cabeceras =$tranx[$i]['niveEcon'].',Cliente/Alumno,Fact. ref.,Pagos ref.,Total Bruto,(-)Pronto p.,(-)Dscto.,I.V.A.,I.C.E.,Total Neto,Tipo descuento,Fecha,Cajero';
					}
