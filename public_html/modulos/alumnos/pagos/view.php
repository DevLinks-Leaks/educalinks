...
$diccionario = array(
    'subtitle'=>array(  VIEW_HOME=>'Pagos',
						VIEW_DEBT=>'Revisión de datos del pago',
						VIEW_DEBT_ANS=>'Respuesta de transacción'
    ),
    'active_menu'=>array(
		'open'  => '{open0}',
        'submenu'  => '{menuRepr07}',
...
function retornar_vista_general($vista, $data=array()) {
    ...
    if($vista==VIEW_MAIN || $vista==VIEW_PASSWORD || $vista==VIEW_HOME || $vista==VIEW_DEBT || $vista==VIEW_DEBT_ANS )
	{   $html = get_main('main');
    }
	else
	{   $html = get_index();
