<?php
	...
	if (!function_exists('para_sist')) {
		function para_sist($para_sist_codi){
			...
			return $_SESSION['parametros'][$para_sist_codi];
		}
	}
	...
	if (!function_exists('get_para_gene')) {
		function get_para_gene($para_codi){
			//Retorna el valor del parámetro del cliente consultado
			include ('dbconf_main.php');		 
			$params_para = array($para_codi);		
			$sql_para = "{call para_cons(?)}";
			$para_resul = sqlsrv_query($conn, $sql_para, $params_para); 
			$row = sqlsrv_fetch_array($para_resul);	 
			return $row;
		}
	}
	...
	if (!function_exists('nota_peri_cual_cons')) {
		function nota_peri_cual_cons($peri_codi, $nota_refe_cabe_codi, $nota){
			for ($i=0;$i<count($_SESSION['equivalencias']);$i++)
			{	if ($_SESSION['equivalencias'][$i]['nota_refe_cab_codi']==$nota_refe_cabe_codi)
				{	if (($nota>=$_SESSION['equivalencias'][$i]['nota_peri_cual_ini']) && ($nota<=$_SESSION['equivalencias'][$i]['nota_peri_cual_fin']))
					{
						return $_SESSION['equivalencias'][$i]['nota_peri_cual_refe'];
					}
				}
			}
			return "NN";
	...
	if (!function_exists('alum_tiene_deuda_vencida')) {
		function alum_tiene_deuda_vencida($alum_codi, $peri_codi){
			// Consulta si un estudiante tiene deuda vencidas tomado desde modulo financiero
			include ('dbconf.php');		 
			$params_deud = array($alum_codi, $peri_codi);		
			$sql_deud="{call cali_repr_show(?,?)}";
			$alum_deud = sqlsrv_query($conn, $sql_deud, $params_deud); 
			$row_alum_deud = sqlsrv_fetch_array($alum_deud);	 
			return $row_alum_deud['tiene_deuda'];
		}
	}
	...
	if (!function_exists('quitarCaracteresEspeciales'))
	...
	{   function validarNI($strCedula,$tipo_iden)
		{	//El decimo Digito es un resultante de un calculo
			...
			if(is_null($strCedula) || empty($strCedula))
			{	//compruebo si que el numero enviado es vacio o null
				echo "Por Favor Ingrese la Cédula";
			}
			else
			{	//caso contrario sigo el proceso
				if ($tipo_iden==3){
				...
				{	echo "Esta Cédula o RUC no corresponde a un Nro de Identidad de Ecuador";
					//echo "Incorrecto"
				}
			}
		}
