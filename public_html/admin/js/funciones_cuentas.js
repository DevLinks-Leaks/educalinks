...
function cuentas_del( dat, alum_codi )
{   $('#modal_alum_cuenta_ask').modal("show");
	document.getElementById('modal_alum_cuenta_ask_body').innerHTML = '<div class="row"><div class="col-md-12">¿Está seguro de querer eliminar la forma de pago? Se perderá la información de esta <b>tarjeta/cuenta bancaria</b>.</div></div>';
	document.getElementById('modal_alum_cuenta_ask_footer').innerHTML = 
	"<button class='btn btn-danger' type='button' onclick=\"cuentas_del_followed('"+dat+"','"+alum_codi+"')\"><span class='fa fa-trash'></span>&nbsp;Eliminar</button>"+
	'<button class="btn btn-default" data-dismiss="modal"><li style="color:red;" class="fa fa-ban"></li>&nbsp;No Eliminar</button>';
}
function cuentas_del_followed( data, alum_codi )
{   $('#modal_alum_cuenta_ask').modal("hide");
    ...
    xmlhttp.open( "POST", 'script_cuentas.php' ,true );
    xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
    xmlhttp.send(data); 
}
function cuentas_apro( dat, alum_codi )
{   $('#modal_alum_cuenta_ask').modal("show");
    document.getElementById('modal_alum_cuenta_ask_body').innerHTML = '<div class="row"><div class="col-md-12">¿Está seguro de querer dar aprobación al <b>Método de pago</b>?</div></div>';
    document.getElementById('modal_alum_cuenta_ask_footer').innerHTML = 
    "<button class='btn btn-success' type='button' onclick=\"cuentas_del_followed('"+dat+"','"+alum_codi+"')\"><span class='fa fa-check'></span>&nbsp;Aprobar</button>"+
    '<button class="btn btn-default" data-dismiss="modal"><li style="color:red;" class="fa fa-ban"></li>&nbsp;No Aprobar</button>';
}
function cuentas_apro_followed( data, alum_codi )
{   $('#modal_alum_cuenta_ask').modal("hide");
    //document.getElementById(div).innerHTML='';
    if (window.XMLHttpRequest)
    {// code for IE7+, Firefox, Chrome, Opera, Safari
