...
{	private $codigo;
	...
	{	$logo_cole = '../../../'.$_SESSION['ruta_foto_logo_index'];
		$this->Image($logo_cole, 5, 8, 20, 23, 'PNG', '', 'C', false, 300, '', false, false, 0, false, false, false);
		$this->Image($this->foto, 185, 8, 16, 18, 'JPG', '', 'C', false, 300, '', false, false, 0, false, false, false);
		$this->SetFont('helvetica', 'B', 7);
		$this->MultiCell(70, 10, '', 0, 'C', 0, 1, '', '', true);
		$this->MultiCell(100, 4, 'INFORME DE EVALUACIÓN DEL APRENDIZAJE Y COMPORTAMIENTO', 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 4, mb_strtoupper($this->periodo,'utf8'), 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 4, mb_strtoupper($this->curso,'utf8'), 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 4, mb_strtoupper(substr($this->nombre.' '.$this->apellido,0,45),'utf8'), 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 4, 'AÑO LECTIVO '.$_SESSION['peri_deta'], 0, 'L', 0, 1, 25, '', true);
...
$tabla_comportamiento.= '<td class="cabecera_notas centrar"><b>ESCALA CUALITATIVA COMPORTAMENTAL</b></td>';
$tabla_comportamiento.= '</tr>';
...
$tabla_cuantitativa.= '<td class="cabecera_notas centrar" colspan="3"><b>EQUIVALENCIAS CUALITATIVAS DE APRENDIZAJE</b></td>';
$tabla_cuantitativa.= '</tr>';
...
{	$tabla_cuantitativa.= '<tr><td class="tabla_informativa" width="20%"> '.
						...
						truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_fin']).'</td><td class="tabla_informativa" width="15%"> ('.
						$row_nota_peri_cual_tipo_view['nota_peri_cual_refe'].')</td><td class="tabla_informativa" width="65%"> '.$row_nota_peri_cual_tipo_view['nota_peri_cual_deta'].
						'</td></tr>';
}
...
$tabla_observaciones.='<tr>';
$tabla_observaciones.='<td class="tabla_informativa" height="48" rowspan="4"> '.$observaciones['nota_obse_deta'].'</td>';
$tabla_observaciones.='</tr>';
...
$asign_ancho = 100-($num_cols*6)-13-2-20;
/*Calificaciones*/
$calificaciones = '
...
$calificaciones.='<td class="cabecera_notas centrar" width="20%">PLAN MEJORA ACADÉMICO</td>';
$calificaciones.='</tr>';
...
{ 	$cc +=1;
	...
	$calificaciones.='<td class="cuerpo_notas" width="20%"></td>';
	$calificaciones.='</tr>';
}
...
<table width="99%">
<tr>
<td width="45%">{$tabla_cuantitativa}<br/></td>
<td width="55%"> {$tabla_comportamiento}</td>
</tr>
<tr>
