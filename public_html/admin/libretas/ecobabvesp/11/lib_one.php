...
{	private $codigo;
	...
	{	$logo_cole = '../../../'.$_SESSION['ruta_foto_logo_libreta'];
		$this->Image($logo_cole, 130, 10, 25, 18, 'PNG', '', 'C', false, 300, '', false, false, 0, false, false, false);
...
{	$tabla_cuantitativa.= '<tr><td class="cuerpo_notas" width="20%"> '.
						...
						truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_fin']).'</td><td class="cuerpo_notas" width="15%"> ('.
						$row_nota_peri_cual_tipo_view['nota_peri_cual_refe'].')</td><td class="cuerpo_notas" width="65%"> '.$row_nota_peri_cual_tipo_view['nota_peri_cual_deta'].
						'</td></tr>';
}
...
{	$params = array('P', $_SESSION['peri_codi']);
	...
	$tabla_proyecto.= '<tr>';
	$tabla_proyecto.= '<td class="cabecera_notas centrar"><b>PROYECTOS</b></td>';
	$tabla_proyecto.= '</tr>';
	$sql="{call nota_peri_cual_tipo_view(?,?)}";
	$nota_peri_cual_tipo_view = sqlsrv_query($conn, $sql, $params);
	while ($row_nota_peri_cual_tipo_view = sqlsrv_fetch_array($nota_peri_cual_tipo_view))
...
$tabla_firmas = '<table>';
$tabla_firmas.= '<tr>';
$tabla_firmas.= '<td class="firmas centrar">__________________________<br/>'.para_sist(5).'<br/>'.para_sist(33).'</td>';
$tabla_firmas.= '<td class="firmas centrar">__________________________<br/>Tutor(a)</td>';
$tabla_firmas.= '<td class="firmas centrar">__________________________<br/>Representante</td>';
$tabla_firmas.= '</tr>';
$tabla_firmas.= '</table>';
/*Contraseñas*/
if (para_sist(7))
...
<td width="40%"> {$tabla_cuantitativa}</td>
<td width="10%"> {$tabla_proyecto}</td>
</tr>
</table>
<br/><br/>
<table width="99%">
...
<td width="49%"> <br/><br/><br/><br/>{$tabla_firmas}</td>
</tr>
</table> 
<table width="50%">
<tr>
