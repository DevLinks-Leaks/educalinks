...
{	private $codigo;
	...
	{	$logo_cole = '../../../'.$_SESSION['ruta_foto_logo_libreta'];
		$this->Image($logo_cole, 130, 10, 22, 20, 'PNG', '', 'C', false, 300, '', false, false, 0, false, false, false);
		...
		//$this->MultiCell(0, 0, 'Código: R8E-03 / Versión: 3 / Nov. 2015', 0, 'L', 0, 1, 230, 10, true);
...
$tabla_observaciones.= '<td class="cabecera_notas centrar"><b>OBSERVACIONES Y SUGERENCIAS</b></td>';
$tabla_observaciones.= '</tr>';
...
$tabla_observaciones.='<tr>';
$tabla_observaciones.='<td class="cuerpo_notas" height="76" rowspan="4"> '.$observaciones['nota_obse_deta'].'</td>';
$tabla_observaciones.='</tr>';
$tabla_observaciones.='</table>';
/*Firmas*/
$tabla_firmas = '<table>';
$tabla_firmas.= '<tr>';
$tabla_firmas.= '<td class="firmas centrar">________________________________<br/>'.para_sist(6).'<br/>'.para_sist(34).'</td>';
$tabla_firmas.= '<td class="firmas centrar">________________________________<br/>Representante</td>';
$tabla_firmas.= '</tr>';
$tabla_firmas.= '</table>';
/*Contraseñas*/
if (para_sist(7))
...
/*Para las recomendaciones de Arcoiris*/
$params = array($alum_codi,$peri_dist_codi,'C');
$sql="{call alum_nota_peri_dist_view_reco(?,?,?)}";
$alum_nota_peri_dist_view_2 = sqlsrv_query($conn, $sql, $params); 
$recomendaciones = '
<table width="99%" border="1" cellpadding="1" cellspacing="0">
  <tr>
	<td class="cabecera_notas" align="center" width="25%">ASIGNATURAS</td>
	<td class="cabecera_notas" align="center" width="5%">PROM.</td>
	<td class="cabecera_notas" align="center" width="5%">EQ.</td>
	<td class="cabecera_notas" align="center" width="25%">RECOMENDACIONES</td>
	<td class="cabecera_notas" align="center" width="40%">PLAN DE MEJORA (¿Qué es lo que el docente planeará para conseguir la mejora con sus estudiantes?)</td>
  </tr>';

while ($row_alum_nota_peri_dist_view_2 = sqlsrv_fetch_array($alum_nota_peri_dist_view_2)) 
{ 	if ($row_alum_nota_peri_dist_view_2["mate_padr"] <0)
	{	$recomendaciones.='<tr><td class="cuerpo_notas" width="25%">';
		if ($row_alum_nota_peri_dist_view_2["mate_padr"] >0)
		{	$recomendaciones.='   ';
		}
			if ($row_alum_nota_peri_dist_view_2["mate_padr"]>0)
			{	$recomendaciones.= ucwords(mb_strtolower($row_alum_nota_peri_dist_view_2['mate_deta'],'UTF-8'));
			}
			else
			{	$recomendaciones.= ' '.mb_strtoupper($row_alum_nota_peri_dist_view_2['mate_deta'],'UTF-8');
			}
		$recomendaciones.='</td>';
		$recomendaciones.='<td width="5%" class="cuerpo_notas centrar ';
		$recomendaciones.= '">';
		if ($row_alum_nota_peri_dist_view_2['PM']>0 and $row_alum_nota_peri_dist_view_2['PM']<>null)
		{	switch ($row_alum_nota_peri_dist_view_2['mate_tipo'])
			{	case "C":
				$recomendaciones.= (truncar($row_alum_nota_peri_dist_view_2['PM'])==0)?'':truncar($row_alum_nota_peri_dist_view_2['PM']);
				break;
				case "D":
				$recomendaciones.= notas_prom_quali($_SESSION['peri_codi'],'D',$row_alum_nota_peri_dist_view_2['PM']);
				break;
				case "P":
				$recomendaciones.= notas_prom_quali($_SESSION['peri_codi'],'P',$row_alum_nota_peri_dist_view_2['PM']);
				break;
			}
		}	            
		$recomendaciones.='</td>';
		$recomendaciones.='<td class="cuerpo_notas centrar" width="5%">'.$row_alum_nota_peri_dist_view_2['nota_peri_cual_refe'].'</td>';
		$recomendaciones.='<td width="25%"></td>';
		$recomendaciones.='<td width="40%"></td>';
		$recomendaciones.='</tr>';
	}
}
...
/*Para la parte de recomendaciones de Arcoiris*/
$tbl_reco=<<<EOF
<style>
.cabecera_notas
{	background-color: #1D00BF;
...
EOF;
$pdf->AddPage();
$pdf->writeHTML($tbl_reco, true, false, false, false, '');
