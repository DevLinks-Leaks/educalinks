...
{	private $codigo;
	...
	{	$logo_cole = '../../../'.$_SESSION['ruta_foto_logo_libreta'];
		...
		$this->MultiCell(90, 5, 'BOLETÍN DE CALIFICACIONES : '.mb_strtoupper($this->periodo,'utf8'), 0, 'L', 0, 1, '', '', true);
...
{	$alum_codi=$alumno['alum_codi'];
	...
	{	if (!alum_tiene_deuda_vencida($alumno['alum_codi'],$_SESSION['peri_codi']))
		{	unset($prom);
			...
			$tabla_firmas.= '<td class="firmas centrar">________________________________<br/>Representante Legal</td>';
			$tabla_firmas.= '</tr>';
			$tabla_firmas.= '</table>';
			/*Contraseñas*/
			if (para_sist(7))
			{	$tabla_contraseñas = '<table width="100%" border="1" cellpadding="1" cellspacing="0">';
				...
				$tabla_contraseñas.= '<td class="cabecera_notas centrar"><b>ESTUDIANTE</b></td>';
