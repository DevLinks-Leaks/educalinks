<?php
	...
	{   
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(0, 1, pasarMayusculas(para_sist(36) . ' '.para_sist(3)));
		$objPHPExcel->getActiveSheet()->getStyle('A1:Q1')->applyFromArray($styleArrayFontEncabezado);
		
		//Periodo distribución
		$periodo_descrp=$cab_row['nivel_2'].', '.$cab_row['nivel_1'];
		...
		//Nivel
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(0, 4, 'TUTOR: '.$row_curs_info['tutor']);
		...
		//Curso
		$curso_descr=$row_curs_peri_info['curs_deta'].' '.$row_curs_info['nive_deta'].' "'.$row_curs_peri_info['para_deta'].'"';
		...
		//Usuario que genera
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(14, 5, PrimeraMayuscula(show_this(10000001)) . ': ' . $_SESSION['usua_codi']);
	 	...
	 	unset($notas_prom);
                ...
                {	$asterisco="";
                    ...
                    for ($j=0;$j<count($columnas);$j++) 
                    {
						...
						$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($w+1, $i+9, ($row_curs_peri_info['nota_refe_cab_tipo']=='C'?truncar($nota):'-'));
		...
		{ 
            ...
            if ($row_curs_peri_info['nota_refe_cab_tipo']=='C')
            {   
                $objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(2, ($i+11), truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_ini']).' - '.(truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_fin'])));
            }
            //BORDES
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(2).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(3).($i+11))->applyFromArray($styleArray);
            ...
            $objPHPExcel->getActiveSheet()->mergeCells('H'.($i+11).':I'.($i+11));
            $objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(7, ($i+11), truncar(contar_notas_porc($notas_prom, $row_nota_peri_cual_tipo_view['nota_peri_cual_ini'], $row_nota_peri_cual_tipo_view['nota_peri_cual_fin'])).' %');
            //Bordes
        ...
        // $i++;
				...
				$objDrawing->setPath('../'.$_SESSION['ruta_foto_logo_libreta']);
	...
	{   
		...
		$objPHPExcel->getActiveSheet()->getStyle('1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->getActiveSheet()->mergeCells('A1:Q1');
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 1, pasarMayusculas(para_sist(36) . ' '.para_sist(3)));
		$objPHPExcel->getActiveSheet()->getStyle('A1:Q1')->applyFromArray($styleArrayFontEncabezado);
		...
		$objPHPExcel->getActiveSheet()->getStyle('2')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->getActiveSheet()->mergeCells('A2:Q2');
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 2, pasarMayusculas(show_this_phrase(20000002)) . ': '.$periodo_descrp);
		$objPHPExcel->getActiveSheet()->getStyle('A2:Q2')->applyFromArray($styleArrayFontEncabezado);
		...
		$objPHPExcel->getActiveSheet()->getStyle('3')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->getActiveSheet()->mergeCells('A3:Q3');
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 3, pasarMayusculas(show_this_phrase(20000005)) . ' '.$_SESSION['peri_deta'].' JORNADA '.mb_strtoupper($row_curs_peri_info['jorn_deta'],'UTF-8'));
		$objPHPExcel->getActiveSheet()->getStyle('A3:Q3')->applyFromArray($styleArrayFontEncabezado);
		...
		//Nivel
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 4, 'TUTOR: '.$row_curs_info['tutor']);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'4')->getFont(1)->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'4')->applyFromArray($styleArrayFont);

		//Curso
		$curso_descr=$row_curs_peri_info['curs_deta'].' '.$row_curs_info['nive_deta'].' "'.$row_curs_peri_info['para_deta'].'"';
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 5, $curso_descr);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'5')->getFont(1)->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'5')->applyFromArray($styleArrayFont);

		//Asignatura
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 6, pasarMayusculas(show_this(10000005)).': '.$row_curs_peri_info['mate_deta']);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'6')->applyFromArray($styleArrayFont);

		//Fecha
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(7, 4, PrimeraMayuscula(show_this(10000003)).': '.get_fecha_ciudad($current_language));
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).'4')->getFont(1)->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).'4')->applyFromArray($styleArrayFont);

		//Usuario que genera
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(7, 5, PrimeraMayuscula(show_this(10000001)) . ': ' . $_SESSION['usua_codi']);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).'5')->getFont(1)->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).'5')->applyFromArray($styleArrayFont);

		//Profesor
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(7, 6, PrimeraMayuscula(show_this(10000002)).': '.$prof_row["prof_nomb"]." ".$prof_row["prof_apel"]);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).'6')->getFont(1)->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).'6')->applyFromArray($styleArrayFont);
		
		//Matriz de calificaciones
		//Número
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 7,'No.');
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'7')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'7')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex(0))->setWidth(5);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).'7')->getFont(1)->setBold(true);
		...
		//Nombre
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(1, 7, pasarMayusculas(show_this(10000015)));
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(1).'7')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(1).'7')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex(1))->setWidth(58.85);
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(1).'7')->getFont(1)->setBold(true);
		...
		for ($i=0;$i<count($columnas);$i++)
		{
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).'7')->getAlignment()->setTextRotation(90);
                    $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w, 7, $columnas[$i][1]);
					$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).'7')->getAlignment()->setWrapText(true);
                    $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).'7')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).'7')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($w))->setWidth(10);
        ...
        {	$asterisco="";
            ...
            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, $i+9, $i+1);
            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(1, $i+9, $asterisco . $filas[$i][1].' '.$filas[$i][2]);
            //Bordes
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).($i+9))->applyFromArray($styleArray);
            //Bordes
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(1).($i+9))->applyFromArray($styleArray);
            ...
            for ($j=0;$j<count($columnas);$j++) 
            {
				...
				// if($_SESSION['directorio']=='liceonaval')
				if($nota<=0 and $_SESSION['directorio']=='liceonaval'){
					// $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w, $i+9, notas_prom_quali($_SESSION['peri_codi'],$row_curs_peri_info['nota_refe_cab_tipo'],0));
					// $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w+1, $i+9, ($row_curs_peri_info['nota_refe_cab_tipo']=='C'?truncar($nota):'-'));
					
				}else{
					// 
					if($row_curs_peri_info['nota_refe_cab_tipo']=='C'){
						$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w, $i+9, ($row_curs_peri_info['nota_refe_cab_tipo']=='C'?truncar($nota):'-'));
						$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).($i+9))->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
					}else{
						$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w, $i+9, notas_prom_quali($_SESSION['peri_codi'],$row_curs_peri_info['nota_refe_cab_tipo'],$nota));
					}
				}
		...
		//El promedio del curso solo se presenta si es una materia cuantitativa
		if($cont_retirados>0)
		{	$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(1, $i+9, "* " . show_this_phrase(20000007));
			$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(1, $i+10, "** " . show_this_phrase(20000003));
		}
		if ($row_curs_peri_info['nota_refe_cab_tipo']=='C')
		{
                    $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w-4, $i+9, show_this_phrase(20000012).':');
					$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w-4).($i+9))->getFont($w-3)->setBold(true);
                    $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w, $i+9, truncar($prom_curso));
					$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).($i+9))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).($i+9))->getFont($w+1)->setBold(true);
                    $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow($w-1, $i+9, notas_prom_quali($_SESSION['peri_codi'],$row_curs_peri_info['nota_refe_cab_tipo'],$prom_curso));
					$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w-1).($i+9))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w-1).($i+9))->getFont($w)->setBold(true);
                    $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).($i+9))->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
		}
		/*//Cuadro de resumen
		$objPHPExcel->getActiveSheet()->mergeCells('A'.($i+11).':B'.($i+11));
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($i+11), pasarMayusculas(show_this(10000018)));
		//Bordes
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(1).($i+11))->applyFromArray($styleArray);
		
		$objPHPExcel->getActiveSheet()->mergeCells('C'.($i+11).':D'.($i+11));
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($i+11), pasarMayusculas(show_this(10000019)));
		//Bordes
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(2).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(3).($i+11))->applyFromArray($styleArray);
		 
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(4, ($i+11), 'ABRE.');
		//Bordes
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(4).($i+11))->applyFromArray($styleArray);
		 
		$objPHPExcel->getActiveSheet()->mergeCells('F'.($i+11).':G'.($i+11));
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(5, ($i+11), pasarMayusculas(show_this_phrase(20000013)));
		//Bordes
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(5).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(6).($i+11))->applyFromArray($styleArray);
		 
		$objPHPExcel->getActiveSheet()->mergeCells('H'.($i+11).':I'.($i+11));
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(7, ($i+11), pasarMayusculas(show_this(10000017)));
		//BORDES
		$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(6).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(8).($i+11))->applyFromArray($styleArray);*/

		/*CUADRO ESPECIAL LICEO NAVAL*/ if($_SESSION['directorio']=='liceonaval'){
			$t=$i;
			$objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), 'ACTIVIDADES INDIVIDUALES INSUMOS');
	        // $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(10).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(13).($i+11))->applyFromArray($styleArray);

	        $objPHPExcel->getActiveSheet()->mergeCells('C'.($t+11).':F'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($t+11), 'ACTIVIDADES GRUPALES INSUMOS');
	        // $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(14).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(15).($i+11))->applyFromArray($styleArray);
	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Pruebas de diferentes estructuras');
	        $objPHPExcel->getActiveSheet()->mergeCells('C'.($t+11).':F'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($t+11), '- Ensayos');

	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Ensayos');
	        $objPHPExcel->getActiveSheet()->mergeCells('C'.($t+11).':F'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($t+11), '- Proyectos');

	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Lecciones orales/escritas');
	        $objPHPExcel->getActiveSheet()->mergeCells('C'.($t+11).':F'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($t+11), '- Trabajos grupales');

	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Informes');
	        $objPHPExcel->getActiveSheet()->mergeCells('C'.($t+11).':F'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($t+11), '- Trabajos grupales de laboratorios');

	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Exposiciones');
	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Trabajo práctico');
	        $t++;
	        $objPHPExcel->getActiveSheet()->mergeCells('A'.($t+11).':B'.($t+11));
	            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($t+11), '- Tareas');
	    }
        /*FINAL CUADRO ESPECIAL*/
        /*
		while ($row_nota_peri_cual_tipo_view = sqlsrv_fetch_array($nota_peri_cual_tipo_view)) 
		{ 
            $i++;
            $objPHPExcel->getActiveSheet()->mergeCells('A'.($i+11).':B'.($i+11));
            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, ($i+11), $row_nota_peri_cual_tipo_view['nota_peri_cual_deta']);
            //BORDES
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(1).($i+11))->applyFromArray($styleArray);
            ...
            if ($row_curs_peri_info['nota_refe_cab_tipo']=='C')
            {   
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(2, ($i+11), truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_ini']).' - '.(truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_fin'])));
            }
            //BORDES
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(2).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(3).($i+11))->applyFromArray($styleArray);
	
            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(4, ($i+11), $row_nota_peri_cual_tipo_view['nota_peri_cual_refe']);
            //BORDES
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(4).($i+11))->applyFromArray($styleArray);

            $objPHPExcel->getActiveSheet()->mergeCells('F'.($i+11).':G'.($i+11));
            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(5, ($i+11), contar_notas_num($notas_prom, $row_nota_peri_cual_tipo_view['nota_peri_cual_ini'], $row_nota_peri_cual_tipo_view['nota_peri_cual_fin']));
            //BORDES
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(5).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(6).($i+11))->applyFromArray($styleArray);
	
            $objPHPExcel->getActiveSheet()->mergeCells('H'.($i+11).':I'.($i+11));
            $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(7, ($i+11), truncar(contar_notas_porc($notas_prom, $row_nota_peri_cual_tipo_view['nota_peri_cual_ini'], $row_nota_peri_cual_tipo_view['nota_peri_cual_fin'])).' %');
            //Bordes
            $objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex(7).($i+11).':'.PHPExcel_Cell::stringFromColumnIndex(8).($i+11))->applyFromArray($styleArray);
        }*/
        // $i++;
            ...
            {
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(0, 1, show_this_phrase(20000004));
            }
