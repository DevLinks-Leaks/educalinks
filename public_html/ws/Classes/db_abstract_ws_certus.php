...
abstract class DB_Abstract_ws_certus{
    ...
    private function open_connection(){    
		include "/conf/dbconf_certus.php";
		include "Classes/conf/dbconf_certus.php";

        $this->connectionInfo = array("Database"=>$db, "UID"=>$uid, "PWD"=>$pwd,"CharacterSet"=>$charset);
        $this->conn= sqlsrv_connect($serverName,$this->connectionInfo);
        if( $this->conn === false){
    ...
    protected function executeSPConsulta(){
        ...
        try{
            ...
            if(!$result){
                ...
                $this->filasDevueltas = count($this->rows)-1;
            }
            sqlsrv_close($result);
            $this->close_connection();			
        }catch(Exception $e){
    ...
    protected function executeSentenciaConsulta(){
        ...
        try{
            $this->open_connection();
            $result = sqlsrv_query($this->conn,$this->query, array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
            if(!$result){
