...
<html lang="es">
	...
	<body class="hold-transition skin-blue sidebar-mini <?php echo $_SESSION['sidebar_status']; ?>">
		<div class="wrapper">
			...
			<?php $Menu = 101; include("Templates/menu.php");?>
			<div class="content-wrapper">
				...
				<section class="content" id="formulario">
					<div id="Main_div" >
						<div id="semana_deta" > 
							<?php include("recurso_new_view.php"); ?>
						</div>
					</div>
				</section>
        ...
        <script type="text/javascript">
           	...
           	$('#recu_fech_publ').datetimepicker({
            format: 'DD/MM/YYYY',
            locale: 'es',
			defaultDate:'<?= date_format($recu_fech_publ, 'd/M/Y' ); ?>',
		 ...
		 $('#recu_fech_ingr').datetimepicker({
            format: 'DD/MM/YYYY',
            locale: 'es',
			defaultDate:'<?= date_format($recu_fech_ingr, 'd/M/Y' ); ?>',
            showTodayButton: true,
            tooltips: {
                today: 'Ir al día actual',
                clear: 'Borrar selección',
   		...
   		$(document).ready(function(){   
			...
			$('#table_cons_Editorialess tbody').on( 'click', 'tr', function () {
					...
					document.getElementById('recu_edit_codi').value=table2.cell( idx, 0).data();
					document.getElementById('recu_edit_deta').value=table2.cell( idx, 1).data(); 
					$('#modal_editorial').modal('hide');
			});
			
			load_ajax_recu_dynamic('dynamic','script_recursos.php','opc=change&tipo_codi='+$('#recu_tipo_codi').val()+'&recu_codi=<?= $recu_codi; ?>');
		});
        ...
        $(document).ready(function(){   
			...
			$('#table_cons_Colecciones tbody').on( 'click', 'tr', function () {
					...
					var idx = table2.row(this).index();
					document.getElementById('recu_cole_codi').value=table2.cell( idx, 0).data();
					document.getElementById('recu_cole_deta').value=table2.cell( idx, 1).data(); 
		...
		$(document).ready(function(){   
			...
			$('#table_cons_Categorias tbody').on( 'click', 'tr', function () {
					...
					var idx = table2.row(this).index();
					recu_cole_codi=table2.cell( idx, 0).data();
					recu_cole_deta=document.getElementById('cate_deta_' + idx).innerHTML; 
					
					var obj = document.getElementById("cate_codi");
					opt = document.createElement("option");
					opt.value = recu_cole_codi;
					opt.text=recu_cole_deta;
					obj.appendChild(opt);
