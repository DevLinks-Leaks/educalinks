...
<html>
	...
	<br>
		<div class="login-box" style='background: transparent; border: transparent;     box-shadow: 0 1px 1px rgba(0,0,0,0.0);'>
			<div class="login-logo">
				<img src="imagenes/clientes/<?= $_SESSION['directorio'];?>/logo_inicial_long.png" alt="Educalinks"  width="80%" height="80%">
			</div>
			<div class="login-box-body"  style='background-color: rgba(255, 255, 255, 0.1);background-color: #FFF;'>
				...
				<form  method="POST"  id="frm_modulo">
					<div class="row">
						<div class="col-xs-12">
							...
							<?php if($_SESSION['certus_acad']){ ?><button style='width:100%' type='submit' class='btn btn-warning' id="btn_acad" onClick="SeleccionarModulo(1)" title="Presione [A] para ir a módulo Académico"><i class='fa fa-graduation-cap'></i>&nbsp;<span style='text-decoration: underline;'>A</span>CADÉMICO</button><?php }?><br><br>
							<?php if($_SESSION['certus_finan']){ if($_SESSION['rol_finan']==1){?><button style='width:100%' type='submit' class='btn btn-success' id="btn_finan" onclick="SeleccionarModulo(2)" title="Presione [F] para ir a módulo Financiero"><i class='fa fa-dollar'></i>&nbsp;<span style='text-decoration: underline;'>F</span>INANCIERO</button><?php }}?><br><br>
							<?php if($_SESSION['certus_biblio']){ if($_SESSION['rol_biblio']==1){?><button style='width:100%' type='submit' class='btn btn-primary' id="btn_biblio" onClick="SeleccionarModulo(4)" title="Presione [B] para ir a módulo Biblioteca"><i class='fa fa-book'></i>&nbsp;<span style='text-decoration: underline;'>B</span>IBLIOTECA</button><?php }}?><br><br>
							<?php if($_SESSION['certus_medic']){ if($_SESSION['rol_medico']==1){?><button style='width:100%' type='submit' class='btn btn-danger' id="btn_medic" onClick="SeleccionarModulo(3)" title="Presione [M] para ir a módulo Médico"><i class='fa fa-medkit'></i>&nbsp;<span style='text-decoration: underline;'>M</span>ÉDICO</button><?php }}?><br><br>
							<!--<?php if($_SESSION['certus_admisiones']){ ?><button type='submit' class='btn btn-app' id="btn_adm" onClick="SeleccionarModulo(5)" title="Presione [D] para ir a módulo Admisiones"><i class='fa fa-child'></i>&nbsp;A<span style='text-decoration: underline;'>D</span>MISIONES</button><?php }?>-->
