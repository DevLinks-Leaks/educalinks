...
{   document.getElementById( 'span_button_return' ).innerHTML='<button class="btn btn-primary fa fa-chevron-left" type="button" ' +
							...
							' onclick="js_clientes_buscar(\'resultado\',\'' +
							document.getElementById( "ruta_html_finan" ).value + 
							'/clientes_externos/controller.php\')">&nbsp;</button>';
	$("#desplegable_busqueda").hide(200);
}
...
{   $("#desplegable_busqueda").show(200);
	...
	data.append('cod_cliente', document.getElementById("txt_cod_cliente").value);
	data.append('id_cliente', document.getElementById("txt_id_cliente").value);
	data.append('nombre_cliente', document.getElementById("txt_nom_cliente").value);
...
function js_clientes_externos_descuento_delete ( codigo, div, url, codigo_cliente )
{   if(confirm("¿Está seguro que desea eliminar el producto actual?"))
	{   var data = new FormData();
		data.append('event', 'delete_descuento');
		data.append('codigo', codigo );
		var xhr = new XMLHttpRequest();
		xhr.open('POST', url , true);
		xhr.onreadystatechange=function()
		{   if (xhr.readyState==4 && xhr.status==200)
			{   $.growl.notice({ title: "Educalinks informa",message: "Proceso realizado." });
				js_clientes_carga_asignacion( codigo_cliente, "modal_asign_body", document.getElementById('ruta_html_finan').value + '/clientes_externos/controller.php' );
			} 
		};
		xhr.send(data);
	}
...
{   var codigo_descto = document.getElementById( 'codigo_descto' );
	...
	{   if(porcentaje_descuento <= 100)
		{   if(confirm("¿Está seguro que desea asignarle el descuento al cliente externo?"))
			{   var data = new FormData();
