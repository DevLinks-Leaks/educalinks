	...
	{   proceso_individual_corriendo = 1;
		...
		{   if (xhr.readyState==4 && xhr.status==200)
			{   document.getElementById(div).innerHTML=xhr.responseText;
				...
				js_convenio_pago_carga_busquedaFacturas('resultadoProceso');
			}
		};
		xhr.send(data);	
	}
...
function js_convenio_pago_validaSave_edited(url)
{	js_convenio_pago_save_edited(url);
	return false;
}
function js_convenio_pago_save_edited(url){
...
function js_convenio_pago_get_fac_pdtes_codi_json( div, url, evento )
{   checkboxes = document.getElementsByName('ckb_codigoDocumento[]');
	...
	{   var chartgrama = new Array();
		...
		js_convenio_pago_get_fac_pdtes_codi_json_followed ( div, url, evento, factura, 0, factura.length, chartgrama, start_time, request_time, per_complete_first );	
	}
	else
...
function js_convenio_pago_get_fac_pdtes_codi_json_followed ( div, url, evento, objeto, indice, obj_len, chartgrama, start_time, request_time, per_complete_first )
{   /*	Esta es una función recursiva. Digamos, por dar una explicación, que estamos ante un caso en el que se va llamara si misma 10 veces.
		...
		-"objeto" es el arreglo con los códigos de las facturas que fueron consultados desde la función la función js_convenio_pago_get_fac_pdtes_codi_json( div, url, evento );
		...
		-"chartgrama" es la pizarra de resultados que se muestra una vez que se terminen las llamadas recursivas. Chartgrama es declarado 
		 en la función js_convenio_pago_get_fac_pdtes_codi_json( div, url, evento );
				...
				{   if ( xhrII.readyState === 4 && xhrII.status === 200 )
					{   if ( indice === 0 ) 
						...
						'<div align="center" style="height:100%;">'+
							...
							'<br>'+ 
							'<br>'+ 
							'<a tabindex="0" class="btn btn-default btn-xs" onclick="js_convenio_pago_detener_envio_SRI(\'pb_envio_por_lote\');" data-toggle="tooltip" title="Detener" data-placement="top"><span class="fa fa-stop"></span></a>'+
							'<a tabindex="0" class="btn btn-default btn-xs" onclick="js_convenio_pago_pausar_envio_SRI(\'pb_envio_por_lote\');" data-toggle="tooltip" title="Pausar" data-placement="right"><span class="fa fa-pause"></span></a>'+
						...
						js_convenio_pago_get_fac_pdtes_codi_json_followed ( div, url, evento, objeto, indice, obj_len, chartgrama, new Date().getTime(), request_time, per_complete_first );
					}
				};
				xhrII.send(data2);
			}
			...
			{   array_chartgrama_envioSRI = chartgrama; /* variable global */
				...
				document.getElementById(div).innerHTML=
					...
					'<div align="center" style="height:100%;">'+
						...
						'<br>'+ 
						'<br>'+ 
						'<a tabindex="0" class="btn btn-default btn-xs" onclick="js_convenio_pago_detener_envio_SRI(\'pb_envio_por_lote\');js_convenio_pago_get_fac_pdtes_codi_json_followed   ( \''+div+'\',\''+ url+'\',\''+ evento+'\',null,'+ indice+','+ obj_len+',null,'+ new Date().getTime()+','+ request_time+','+ per_complete_first+');" data-toggle="tooltip" title="Detener" data-placement="top"><span class="fa fa-stop"></span></a>'+
						'<a tabindex="0" class="btn btn-default btn-xs" onclick="js_convenio_pago_continuar_envio_SRI(\'pb_envio_por_lote\');js_convenio_pago_get_fac_pdtes_codi_json_followed ( \''+div+'\',\''+ url+'\',\''+ evento+'\',null,'+ indice+','+ obj_len+',null,0,'+ request_time+','+ per_complete_first+');" data-toggle="tooltip" title="Continuar" data-placement="right"><span class="fa fa-play"></span></a>'+
						'<div class="progress progress-striped">'+
...
function js_convenio_pago_envio_facturasPorLote( div, url )
{   if ( proceso_corriendo != 0 )
	...
	{   document.getElementById(div).innerHTML=
		...
		var obj = js_convenio_pago_get_fac_pdtes_codi_json ( div, url, 'send_to_sri_all' );
	}	
}
function js_convenio_pago_autorizar_facturasPorLote( div, url )
{   if ( proceso_corriendo != 0 )
		$.growl({ title: "Educalinks informa",message: "Ya hay un lote de comprobantes electrónicos en proceso de envío. Por favor, espere." });
	else
	{   document.getElementById(div).innerHTML=
		...
		'</div>';
		js_convenio_pago_limpiar_log_envio_lote(  );
		var obj = js_convenio_pago_get_fac_pdtes_codi_json ( div, url, 'autorizar_to_sri_all' );
	}
}
function js_convenio_pago_limpiar_log_envio_lote(  )
...
function js_convenio_pago_detener_envio_SRI(progress_bar)
{   proceso_corriendo = 0;
...
function js_convenio_pago_pausar_envio_SRI(progress_bar)
{   proceso_corriendo = 2;
	document.getElementById( 'sms_estado_envio' ).innerHTML = "<span>Pausando envío...</span>";
}
function js_convenio_pago_continuar_envio_SRI(progress_bar)
{   proceso_corriendo = 1;
}
function js_convenio_pago_to_excel_busquedaFacturas( evento, tipo_reporte )
...
{   document.getElementById(div).innerHTML='<br><div align="center" style="height:100%;"><i style="font-size:large;color:#E55A2F;" class="fa fa-cog fa-spin"></i></div>';
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('POST', document.getElementById('ruta_html_finan').value + '/convenio_pago/controller.php' , true);
    xhr.onreadystatechange=function()
    {   if (xhr.readyState==4 && xhr.status==200)
        {   document.getElementById(div).innerHTML=xhr.responseText;
            $(".detalle").tooltip({
...
function js_convenio_pago_check_opc_avanzadas()
{   var ckb_opc_adv = document.getElementById("ckb_gestionFactura_opc_adv").checked;
    if(ckb_opc_adv)
...
function js_convenio_pago_check_tneto()
{    var chk_tneto = document.getElementById("chk_tneto").checked;
    if(chk_tneto)
...
function js_convenio_pago_check_fechadeuda()
...
function js_convenio_pago_check_fechaAut()
...
function js_convenio_pago_select_all2 (  )
...
function js_convenio_pago_select_all ( source )
{   checkboxes = document.getElementsByName('ckb_codigoDocumento[]');
    for(var i = 0, n = checkboxes.length; i < n; i++ )
...
function js_convenio_pago_select_check_ind ( source, num_linea )
{   var marcar = 'si';
	checkboxes = document.getElementsByName('ckb_codigoDocumento[]');
