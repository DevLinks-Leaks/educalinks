...
$(document).ready(function() {
    ...
    $('#facturasPendiente_table').DataTable({
		...
		"columnDefs": [
			{className: "dt-body-center"  , "targets": [0], "orderable": false },
		...
		{   if (xhr.readyState==4 && xhr.status==200)
			{   if ( xhr.responseText.length > 0 )
				{	respuesta = JSON.parse(xhr.responseText);
					document.getElementById(div).innerHTML = respuesta.mensaje;
				}
				else
				{   document.getElementById(div).innerHTML = '<div class="callout callout-danger">'+
								'<h4><strong><li class="fa fa-exclamation"></li> Convenio de Pago</strong></h4>'+
								'No se pudo hacer conexión con el servidor. Por favor, inténtelo nuevamente.'+
							'</div>';
				}
	...
	{   document.getElementById(div).innerHTML=
		"<div id='facturasGeneradas' class='form-group'>"+
			"<div class='callout' role='alert'>"+
				"<h4><i class='icon fa fa-info-circle'></i> No hay deudas seleccionadas</h4>"+
				"<p>En este momento no hay deudas seleccionadas pendiente de pago.</p>"+
			"</div>"+
		"</div>";
	}
}
...
{   /*	Esta es una función recursiva. Digamos, por dar una explicación, que estamos ante un caso en el que se va llamara si misma 10 veces.
	...
	*/
	...
	{   document.getElementById(div).innerHTML='<div class="callout callout-info">'+
					...
					'</div>';		
		proceso_corriendo = 0;
		js_convenio_pago_carga_busquedaFacturas( 'resultadoProceso', url );
	}
	else
		...
		{   if( proceso_corriendo === 1 )
					...
					{   if ( indice === 0 ) 
						...
						{   var estado_respuesta = JSON.parse(xhrII.responseText);
							...
							{   chartgrama[ estado_respuesta.estado ] = chartgrama [ estado_respuesta.estado ] + 1;
								...
								container.innerHTML = estado_respuesta.detalle;
								
								if (estado_respuesta.estado == 'PAGADO' )
						...
						'<div align="center" style="height:100%;">'+
							...
							'</div>'+
						'</div>'+
						'<span style="font-size:xx-small;" align="left">Pagando factura ref. ' + objeto[indice] + ' y enviando a la bandeja de envío al SRI.</span><br>'+
			...
			{   array_chartgrama_envioSRI = chartgrama; /* variable global */
				...
				document.getElementById(div).innerHTML=
					...
					'</div>'+
					'<span style="font-size:xx-small;" align="left">Pagando factura ref. ' + objeto[indice] + ' y enviando a la bandeja de envío al SRI.</span><br>'+
					'<span style="font-size:xx-small;" align="left">' + ( indice ) + ' facturas pagadas de ' + obj_len + '.</span>'+
					'<br>'+ 
					'<br>'+ 
		...
		{   document.getElementById(div).innerHTML=
				"<div id='facturasGeneradas' class='form-group'>"+
					"<div class='callout callout-success' role='alert'>"+
						"<h4><i class='icon fa fa-check'></i> Pago de facturas completado</h4>"+
						"<ul>"+
							"<li><strong>" + chartgrama['PAGADO'] + "</strong> facturas fueron pagadas</li>"+
							"<li><strong>" + chartgrama['ERROR'] + "</strong> facturas tuvieron errores </li>"+
						"</ul>"+
					"</div>"+
				"</div>";
			if (chartgrama['PAGADO'] > 0 )
				document.getElementById( 'span_log_lote_autorizadas' ).innerHTML="<span class='label label-danger'> " + chartgrama['PAGADO'] + "</span>";
			...
			{	if ( proceso_corriendo === 1 )
				{   var sms_notify = "";
					if (obj_len > 1)
						sms_notify = "¡Se han pagado " + ( indice ) + " facturas de convenio de pago!"
					else
						sms_notify = "¡Pago de facturas de convenio de pago completado!"
					
					var notification = new Notification('Educalinks', {
						icon: document.getElementById('ruta_imagenes_common').value + "/favicon.png",
						body: sms_notify,
					});
					var ruta_notify = document.getElementById('ruta_html_finan') + "/../../../finan/gestionFacturas/";
...
function js_convenio_pago_pagar_PorLote( div, url )
{   if ( proceso_corriendo != 0 )
		$.growl({ title: "Educalinks informa",message: "Ya hay un lote de deudas en proceso de generar pagos y facturas. Por favor, espere." });
	else
	{   document.getElementById(div).innerHTML=
		'<br>'+
		'<div align="center" style="height:100%;">'+
		...
		var obj = js_gestionFactura_get_fac_pdtes_codi_json ( div, url, 'send_deuda_cp_all' );
	}
}
...
{	$('.nav-tabs a[href="#modal_lote_body"]').tab('show');
	...
	document.getElementById( 'div_log_lote_errores' ).innerHTM = "";
...
{   proceso_corriendo = 0;
	document.getElementById( 'sms_estado_envio' ).innerHTML = "<span>Deteniendo generación de pagos y facturas...</span> las deudas procesadas ya tienen registrado pago y factura en el sistema.";
}
function js_gestionFactura_pausar_envio_SRI(progress_bar)
{   proceso_corriendo = 2;
	document.getElementById( 'sms_estado_envio' ).innerHTML = "<span>Pausando generación de pagos y facturas...</span>";
}
...
{   if ( $('#span_codigoDocumento_head1').hasClass('fa-square-o') )
	...
	{   document.getElementById('tr_row_head').style.backgroundColor = '#ffc';
		document.getElementById('btn_convenio_pago_pay').disabled = false;
