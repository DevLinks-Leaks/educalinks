...
{   var spanish_in="//cdn.datatables.net/plug-ins/f2c75b7247b/i18n/Spanish.json";
    ...
    {   if ( xhr.readyState === 4 && xhr.status === 200 )
        {   document.getElementById( div_modal ).innerHTML = xhr.responseText;
			...
			$("#numeroIdentificacion_busq").on('keyup', function (e) {
				if (e.keyCode == 13) {
					js_persona_select_user_searchlist_search(document.getElementById('cmb_per_consulta_tipo_persona').value,
														 'numeroIdentificacion','resultadoBusqueda',
														 document.getElementById('ruta_html_common').value + '/persona/controller.php');
				}
			});
			$("#nombre_busq").on('keyup', function (e) {
				if (e.keyCode == 13) {
					js_persona_select_user_searchlist_search(document.getElementById('cmb_per_consulta_tipo_persona').value,
														 'nombres','resultadoBusqueda',
														 document.getElementById('ruta_html_common').value + '/persona/controller.php');
				}
			});
			shortcut.add("Enter", function() {
				...
				'type':'keyup',
				'target':document.getElementById('btn_busq_persona_2_by_id')
			});
			shortcut.add("Enter", function() {
				js_persona_select_user_searchlist_search(document.getElementById('cmb_per_consulta_tipo_persona').value,
														 'nombres','resultadoBusqueda',
				...
				'type':'keyup',
				'target':document.getElementById('btn_busq_persona_2_by_name')
			});
			$(".move").keydown(
				function(e)
				{    
